...
// Set the base path to the directory of the current script
$base_path = __DIR__; // This gives the directory where the current script is located
...
$query = "
     SELECT 
         wq.id,
         wq.repair_request_id,
         wq.quotation_date,
         wq.amount,
         wq.quotation_attachment,
         wq.remarks,
         wq.approved_by,
         wq.approval_date,
         wq.payment_status
     FROM 
         weight_scales_quotations wq

         JOIN 
        weight_scales_requests rr ON wq.repair_request_id = rr.id
    WHERE 
...
function updatePaymentStatus(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            fetch(`update_payment_status_scale.php?id=${id}`, {
