...
<div class="dashboard">
    ...
    <section class="details">
        <div class="table-container">
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                $query = "SELECT id FROM fire_extinguishers WHERE serial_number = ?";
                ...
                if ($row) {
                    ...
                    $insertQuery = "INSERT INTO fire_extinguishers_history (item_id, maintenance_type, maintenance_date, next_scheduled_maintenance) VALUES (?, ?, ?, ?)";
                ...
                Swal.fire({
                    ...
                    window.location.href = 'maintenance-fire.php'; // Redirect if needed
                });
            </script>";
                    } else {
                        echo "<script>
    ...
    LEFT JOIN 
        fire_extinguishers_history feh ON fe.id = feh.item_id
    WHERE 
        fe.enter_by = ?
    ORDER BY 
        fe.id
";
    ...
    try {
        ...
        // Get the table content area
        let content = "";

        // Construct table rows dynamically
        content += `<tr><td><strong>Platform:</strong></td><td>${data.platform || 'N/A'}</td></tr>`;
        content += `<tr><td><strong>Serial Number:</strong></td><td>${data.serial_number || 'N/A'}</td></tr>`;
        content += `<tr><td><strong>Type:</strong></td><td>${data.type || 'N/A'}</td></tr>`;
        content += `<tr><td><strong>Capacity:</strong></td><td>${data.capacity || 'N/A'}</td></tr>`;
        content += `<tr><td><strong>Status:</strong></td><td>${data.status || 'N/A'}</td></tr>`;
        content += `<tr><td><strong>Maintenance Date:</strong></td><td>${data.maintenance_date || 'N/A'}</td></tr>`;
        content += `<tr><td><strong>Next Scheduled Maintenance:</strong></td><td>${data.next_scheduled_maintenance || 'N/A'}</td></tr>`;
        ...
        console.error("Error in showPopup function:", error);
    }
}
