...
<body>
    ...
    $sql = "SELECT f.id, f.serial_number, f.service_type, f.request_date, f.priority, f.model, f.description, f.status, f.station_name, 
    f.platform_number, f.scheduled_service_date, f.service_date, f.repair_details, f.cost_estimation, f.quotation_id, f.invoice_id, 
    o.invoice_attachment, w.quotation_attachment FROM weight_scales_requests f 
    INNER JOIN weight_scales_invoice o ON f.id = o.repair_request_id
    INNER JOIN weight_scales_quotations w ON f.id= w.repair_request_id
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo '    <td >
                                    <button class="action-button" onclick="openModal(' . $row['id'] . ', \'' . $row['serial_number'] . '\', \'' .
                                     $row['service_type'] . '\', \'' . $row['request_date'] . '\', \'' . $row['scheduled_service_date'] . '\', \'' . $row['priority'] .
                                      '\',\'' . $row['model'] . '\',\'' . $row['description'] . '\',\'' . $row['status'] . '\',\'' . $row['service_date'] . '\',\'' 
                                      . $row['repair_details'] . '\',\'' . $row['cost_estimation'] . '\',\'' . $row['quotation_id'] . '\',\'' . $row['invoice_id'] . 
                                      '\',\'' . $row['station_name'] . '\',\'' . $row['platform_number'] . '\')">View</button>
    ...
    <script>
        function openModal(id, serialNumber, serviceType, requestDate, scheduledServiceDate, priority, model, description, status, 
