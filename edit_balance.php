...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($serial_number && $brand && $model && $capacity && $location && $installation_date &&
        $condition && $status && $display_type && $power_source && $weight && $inspection_frequency && $station_id) {
        
        // Prepare the SQL UPDATE query
        $query = "UPDATE weight_scales SET
                    brand = ?, model = ?, capacity = ?, location = ?, installation_date = ?,
                    conditions = ?, status = ?, display_type = ?, power_source = ?, weight = ?, inspection_frequency = ?,
                    additional_notes = ?, station_name = ?
                  WHERE serial_number = ?";

        // Prepare statement
        if ($stmt = mysqli_prepare($conn, $query)) {
            // Bind parameters
            mysqli_stmt_bind_param($stmt, 'ssssssssssssss', $brand, $model, $capacity, $location, $installation_date,
            $condition, $status, $display_type, $power_source, $weight, $inspection_frequency, 
            $additional_notes, $station_id, $serial_number);
...
// Fetch existing balance details for the given serial_number
if (isset($_GET['serial_number'])) {
    $serial_number = $_GET['serial_number'];
    $query = "SELECT * FROM weight_scales WHERE serial_number = '$serial_number'";
...
$platformsQuery = "SELECT platform_number, StationID FROM platform"; 
$platformsResult = mysqli_query($conn, $platformsQuery);
$platforms = [];
while ($platformRow = mysqli_fetch_assoc($platformsResult)) {
    $platforms[$platformRow['StationID']][] = $platformRow['platform_number'];
}
  ...
  <div class="form-container">
    ...
    <input type="hidden" name="serial_number" value="<?php echo $row['serial_number']; ?>">
      ...
      <div class="form-group">
        ...
        <input type="display_type" id="display_type" name="display_type" value="<?php echo $row['display_type']; ?>" required>
      </div>
      <div class="form-group">
        <label for="power_source">Power Source:</label>
        <input type="power_source" id="power_source" name="power_source" value="<?php echo $row['power_source']; ?>" required>
      </div>
      <div class="form-group">
