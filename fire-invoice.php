...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['invoiceId'])) {
    ...
    try {
        ...
        $updateInvoiceQuery = "
            UPDATE fire_extinguishers_invoice 
        ...
        $getRequestIdQuery = "SELECT repair_request_id FROM fire_extinguishers_invoice WHERE id = ?";
        ...
        $updateRequestQuery = "UPDATE fire_extinguishers_requests SET status = 'Completed' WHERE id = ?";
        $stmt3 = mysqli_prepare($conn, $updateRequestQuery);
        mysqli_stmt_bind_param($stmt3, "i", $repairRequestId);
        mysqli_stmt_execute($stmt3);

        $updatequatationQuery = "UPDATE fire_extinguishers_quotations SET payment_status = 'Paid' WHERE repair_request_id = ?";
