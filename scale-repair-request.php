...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Retrieve form data
    $serial_number = isset($_POST['weight-scale-serial']) ? $_POST['weight-scale-serial'] : '';
    ...
    $query = "INSERT INTO weight_scales_requests (serial_number,model, station_name, platform_number, service_type, description, priority, request_date, send_by) 
              VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), ?)";

    // Use prepared statements to prevent SQL injection
    if ($stmt = $conn->prepare($query)) {
        $stmt->bind_param('ssssssss', $serial_number, $model, $station_name, $platform, $service, $description, $priority, $sendBy);
...
<div class="form-container">
    <h1 class="form-title">Weight Scale<br>Repair Request Form</h1>
    <form action="#" method="post">
        
<!-- Serial Number Field -->
<div id="weight-scale-serial" class="form-group">
            <label for="weight-scale-serial-input">Serial Number:</label><br>
            <input type="text" name="weight-scale-serial" id="weight-scale-serial-input" value="<?= htmlspecialchars($serial_number); ?>" readonly>
        </div> 
        
        <label for="model-input">model:</label><br>
            <input type="text" name="model" id="model-input" value="<?= htmlspecialchars($model); ?>" readonly><br><br>
