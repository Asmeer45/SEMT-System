...
$sql = "SELECT id, serial_number, service_type, request_date, priority, type, description, status, station_name, platform_number FROM fire_extinguishers_requests WHERE status = 'pending'ORDER BY id DESC";
$result = $conn->query($sql);

// Fetch all request IDs that exist in the fire_quotations table
$quotation_sql = "SELECT repair_request_id FROM fire_extinguishers_quotations";
...
<button class="action-button1" onclick="openModal(' . $row['id'] . ', \'' . $row['serial_number'] . '\', \'' . $row['service_type'] . '\', \'' . $row['request_date'] . '\', \'' . $row['priority'] . '\',\'' 
                                    . $row['type'] . '\',\'' . $row['description'] . '\',\'' . $row['status'] . '\',\'' . $row['station_name'] . '\',\'' . $row['platform_number'] . '\')">View</button>
                                <a href="fire-quotations.php?request_id=' . $row['id'] . '">
    ...
    <div id="taskModal" class="modal">
        <div class="modal-content">
            ...
            <button class="action-button blue-button" onclick="closeModal()">Close</button>
        </div>
    </div>

    <script>
        function openModal(id, serialNumber, serviceType, requestDate, priority, type, description, status, stationName, platformNumber) {
